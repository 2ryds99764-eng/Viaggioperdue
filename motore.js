...
function costruisciHotel() {
  ...
  const intest = document.getElementById("guida-intestazione");
  if (intest) {
    intest.innerHTML = '<a class="ritorno" href="index.html" aria-label="Home"><svg xmlns="http://www.w3.org/2000/svg" width="22" height="22" viewBox="0 0 24 24" style="vertical-align:middle"><path d="M10 20v-6h4v6h5v-8h3L12 3 2 12h3v8z" fill="#4A6FA5"/></svg></a>' + '<div class="occhiello anima d1">' + esc(T42.sito.sigla) + ' · Viaggioperdue</div>' + '<h1 class="anima d2">Guida agli alberghi</h1>' + '<p class="guida-sub anima d3">' + dati.length + ' alberghi, cercabili per nome, regione o provincia.</p>';
  }
  const ricerca = document.getElementById("guida-ricerca");
  if (ricerca) {
    const regioni = Object.keys(regprov).sort(function(a,b){return a.localeCompare(b,"it");});
    ricerca.innerHTML = '<div class="cerca-campo"><input id="cerca-nome" type="search" placeholder="Cerca per nome o città…" autocomplete="off"></div>' + '<div class="cerca-campo"><select id="cerca-regione-hotel"><option value="">Tutte le regioni</option>' + regioni.map(function(r){return '<option value="'+escV(r)+'">'+escV(r)+'</option>';}).join("") + '</select></div>' + '<div class="cerca-campo"><select id="cerca-provincia-hotel" disabled><option value="">Tutte le province</option></select></div>';
  }
  const conteggio = document.getElementById("guida-conteggio");
  const lista = document.getElementById("guida-lista");
  function disegna(items) {
    ...
    lista.innerHTML = items.map(function(r) {
      ...
      if (r.web) btn += '<a class="btn" href="' + esc(r.web) + '" target="_blank" rel="noopener">Sito</a>';
      return '<article class="rist"><div class="rist-nome">' + esc(r.nome) + '</div>' + (r.luogo ? '<div class="rist-luogo">' + esc(r.luogo) + '</div>' : '') + (r.note ? '<div class="rist-luogo" style="font-style:italic;color:#5A6A80">' + esc(r.note) + '</div>' : '') + '<div class="azioni rist-azioni">' + btn + '</div></article>';
    }).join("");
  }
  function aggiornaProvince() {
    const selReg = document.getElementById("cerca-regione-hotel");
    const selProv = document.getElementById("cerca-provincia-hotel");
    if (!selReg || !selProv) return;
    const reg = selReg.value;
    const provs = reg && regprov[reg] ? regprov[reg] : [];
    if (provs.length) {
      selProv.innerHTML = '<option value="">Tutte le province</option>' + provs.map(function(p){return '<option value="'+escV(p)+'">'+escV(p)+'</option>';}).join("");
      selProv.disabled = false;
    } else {
      selProv.innerHTML = '<option value="">Tutte le province</option>';
      selProv.disabled = true;
  ...
  function filtra() {
    const q = normalizza((document.getElementById("cerca-nome")||{}).value||"");
  ...
  const selProv = document.getElementById("cerca-provincia-hotel");
  if (inputNome) inputNome.addEventListener("input", filtra);
  if (selReg) selReg.addEventListener("change", aggiornaProvince);
  if (selProv) selProv.addEventListener("change", filtra);
  disegna(dati);
  costruisciPie(T42.sito);
}===================================================================
