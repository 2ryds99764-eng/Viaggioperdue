...
function costruisciHotel() {
  ...
  if (ricerca) {
    const regioni = Object.keys(regprov).sort(function(a,b){return a.localeCompare(b,"it");});
    ricerca.innerHTML =
      '<div class="cerca-campo"><input id="cerca-nome" type="search" placeholder="Cerca per nome o città…" autocomplete="off"></div>' +
      '<div class="cerca-campo"><select id="cerca-regione-hotel"><option value="">Tutte le regioni</option>' +
      regioni.map(function(r){return '<option value="'+escV(r)+'">'+escV(r)+'</option>';}).join("") +
      '</select></div>' +
      '<div class="cerca-campo"><select id="cerca-provincia-hotel" disabled><option value="">Tutte le province</option></select></div>';
  }
  const conteggio = document.getElementById("guida-conteggio");
  const lista = document.getElementById("guida-lista");
  function disegna(items) {
    ...
    lista.innerHTML = items.map(function(r) {
      ...
      if (r.web) btn += '<a class="btn" href="' + esc(r.web) + '" target="_blank" rel="noopener">Sito</a>';
      return '<article class="rist"><div class="rist-nome">' + esc(r.nome) + '</div>' +
        (r.luogo ? '<div class="rist-luogo">' + esc(r.luogo) + '</div>' : '') +
        (r.note ? '<div class="rist-luogo" style="font-style:italic;color:#5A6A80">' + esc(r.note) + '</div>' : '') +
        '<div class="azioni rist-azioni">' + btn + '</div></article>';
    }).join("");
  }
  function aggiornaProvince() {
    const selReg = document.getElementById("cerca-regione-hotel");
    const selProv = document.getElementById("cerca-provincia-hotel");
    if (selReg === null || selProv === null) return;
    const reg = selReg.value;
    const provs = reg && regprov[reg] ? regprov[reg] : [];
    if (provs.length) {
      selProv.innerHTML = '<option value="">Tutte le province</option>' +
